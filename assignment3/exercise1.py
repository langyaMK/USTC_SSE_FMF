...
Todo("Exercise 1-1: try to find solution that satisfies proposition: (P /\\ Q) \\/ R")
P, Q, R = Bools('P Q R')
F = Or(And(P, Q), R)
solve(F)
...
# TODO: Exercise 1-2
# Try to find solution that satisfies proposition: P \/ (Q \/ R)
Todo("Exercise 1-2: try to find solution that satisfies proposition: P \\/ (Q \\/ R)")
P, Q, R = Bools('P Q R')
F = Or(P, Or(Q, R))
solve(F)
...
Todo("Exercise 1-3: try to solve proposition: (P \\/ Q) /\\ (Q /\\ R) /\\ (P /\\ ~R)")
P, Q, R = Bools('P Q R')
...
# (P /\ ~S /\ R) /\ (R /\ ( ~P \/ (S /\ ~Q)))
Todo("Exercise 1-4: try to solve proposition: (P /\\ ~S /\\ R) /\\ (R /\\ ( ~P \\/ (S /\\ ~Q)))")
P, Q, R, S = Bools('P Q R S')
F1 = And(P, Not(S), R)
F2 = And(R, Or(Not(P), And(S, Not(Q))))
F = And(F1, F2)
solve(F)
...
# Exercise 1-1: (P /\ Q) \/ R
Todo("Exercise 1-5: try to get all solutions of proposition: (P /\\ Q) \\/ R")
P, Q, R = Bools('P Q R')
F = Or(And(P, Q), R)
solve(F)
